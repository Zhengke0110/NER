...
def encode_tags(tags: List[List[str]], encodings: Any, tag2id: Dict[str, int], 
                max_seq_len: int = 510) -> List[List[int]]:
    """
    编码标签序列，处理tokenizer的offset mapping
    ...
    for doc_labels, doc_offset in zip(labels, encodings.offset_mapping):
        # 创建全由-100组成的数组（-100表示忽略的标签）
        doc_enc_labels = np.ones(len(doc_offset), dtype=int) * -100
        arr_offset = np.array(doc_offset)
        
        # 处理序列长度超限的情况
        if len(doc_labels) >= max_seq_len:
            logger.warning(f"文档标签长度 {len(doc_labels)} 超过最大长度 {max_seq_len}，将截断")
            doc_labels = doc_labels[:max_seq_len]
        
        # 只为真实token设置标签（offset-mapping中 [0,0] 表示特殊token）
        try:
            # 找到有效的token位置
            valid_positions = (arr_offset[:, 0] == 0) & (arr_offset[:, 1] != 0)
            valid_count = np.sum(valid_positions)
            
            # 确保标签数量不超过有效位置数量
            actual_labels = doc_labels[:min(len(doc_labels), valid_count)]
            doc_enc_labels[valid_positions][:len(actual_labels)] = actual_labels
            
        except (IndexError, ValueError) as e:
            logger.warning(f"处理标签编码时出错: {e}")
            continue
        
        encoded_labels.append(doc_enc_labels.tolist())
    
    logger.info(f"完成标签编码，共 {len(encoded_labels)} 个序列")
